...
class BatchInwardController extends GetxController {
  ...
  Future<void> _fetchTareProductsList() async {
    var response = await dashboardController.callApi(apiCall: ()=>connectHelper.getTareList());
  ...
  Future<void> _fetchBatchDetail(String batchId) async {
    var response = await dashboardController.callApi(apiCall: ()=>connectHelper.getBatchDetails(batchId));
    ...
    if (dropdownProducts.isNotEmpty) {
      ...
      manualCtrl.manualTare.value = selectedModelProduct.value?.tareWeight.toString();
      manualCtrl.tareCtrl.text = selectedModelProduct.value?.tareWeight.toString() ?? '0';
  ...
  Future<void> onPauseOrStop({required String pauseOrStop}) async {
    ...
    for (var item in productList) {
      ...
      // Prepare safe values
      final double tare = double.tryParse(item.tareWeight ?? "0") ?? 0;
      final double gross = double.tryParse(item.grossWeight ?? "0") ?? 0;

      // If netWeight is null or invalid → auto calculate
      final double net =
          double.tryParse(item.netWeight ?? "") ??
          (gross - tare); // <-- FIXED HERE
    ...
    // API CALL
    var response = await dashboardController.callApi(apiCall: ()=>connectHelper.batchProductStore(
      batch_inward_model: data,
    ));
  ...
  Future<void> addToList() async {
    ...
    productList.insert(
      ...
      Barcodes(
        ...
        grossWeight: manualCtrl.manualGross.value ?? '0',
        tareWeight: isTareWeightOff.value
            ? '0'
            : manualCtrl.manualTare.value ?? '0',
        netWeight: manualCtrl.manualNet.value ?? '0',
    ...
    final manualGTNFields = {
      "Gross Weight": manualCtrl.manualGross.value,
      "Tare Weight": manualCtrl.manualTare.value,
      "Net Weight": manualCtrl.manualNet.value,
    };
    final manualNFields = {
      "Net Weight": manualCtrl.manualNet.value,
    };
    ...
    final labelFields = isTareWeightOff.value ? {...combinationFields, ...manualNFields} : {...combinationFields, ...manualGTNFields};
    LabelFormat selectedLabelFormat = LabelFormat.Large;
    number = int.tryParse(selectedModelProduct.value?.labelId ?? '1') ?? 3;
